...
// recent enough that the sync is still running?
//
...
/**
 * @param {{ api: { getSyncLog: (limit: number) => Promise<object[]> },
 *   now?: Date, thresholdHours?: number }} opts
 ...
 */
export async function checkFreshness({ api, now = new Date(), thresholdHours = DEFAULT_THRESHOLD_HOURS }) {
  ...
  let rows;
  try {
    rows = await api.getSyncLog(1);
  } catch (err) {
    return {
      ok: false,
      message: `Could not read SyncLog (${err.name ?? 'Error'}${err.action ? `, ${err.action}` : ''}), so nobody ` +
        'can tell whether the COROS sync is running. The sync writes through the same API, so it is ' +
  ...
  if (!newest) {
    return { ok: false, message: `SyncLog is empty: the COROS sync has never recorded a run (${limit}).` };
  }
  ...
  if (Number.isNaN(started)) {
    return {
      ok: false, newest,
      message: `The newest SyncLog row, ${newest.run_id}, has an unreadable started_at ("${newest.started_at}").`,
    };
  }

  const ageHours = (now.getTime() - started) / HOUR;
  const described = `newest SyncLog row ${newest.run_id} started ${newest.started_at}, ` +
    `${ageHours.toFixed(1)} h ago, status ${newest.status || '(blank)'}; ${limit}`;
  if (ageHours > thresholdHours) {
    return {
      ok: false, newest, ageHours,
      message: `The COROS sync has not run for ${ageHours.toFixed(1)} hours: ${described}. ` +
        'Check Actions → coros-sync: is its schedule still enabled, and are runs being cancelled or timing out?',
    };
  }
  return { ok: true, newest, ageHours, message: `The COROS sync is running: ${described}.` };
}

/** `--now <ISO>` and `--threshold-hours <n>`, for proving the check trips. */
export function parseDeadmanArgs(argv, env = {}) {
  ...
  const thresholdHours = thresholdArg === undefined || thresholdArg === '' ? DEFAULT_THRESHOLD_HOURS : Number(thresholdArg);
  if (!(thresholdHours > 0)) throw new Error(`--threshold-hours must be a positive number, got "${thresholdArg}"`);
  return { now, thresholdHours };
}
