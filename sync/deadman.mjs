...
//
// Its output names run IDs, timestamps and a status, never COROS data, so it
...
try {
  const { now, thresholdHours } = parseDeadmanArgs(process.argv.slice(2), process.env);
  const api = createThriveApi(loadThriveApiConfig());
  const result = await checkFreshness({ api, now, thresholdHours });
  (result.ok ? console.log : console.error)(result.message);
  process.exitCode = result.ok ? 0 : 1;
} catch (err) {
  // A config error names the missing secrets; nothing here carries COROS data.
